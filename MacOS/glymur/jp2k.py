...
# Modified by JH to work with Python 2.5 & 2.7, but probably not 3
...
if sys.hexversion < 0x02070000:
    # Taken from Python2.7 implementation
...
class Jp2k(Jp2kBox):
    ...
    def parse(self):
        ...
        with open(self.filename, 'rb') as fptr:
            ...
            box_id = values[1]
            signature = values[2:]
            if (((box_length != 12) or (box_id != 'jP  ') or
                 (signature != (13, 10, 135, 10)))):
                msg = '%s is not a JPEG 2000 file.' % self.filename
                raise IOError(msg)
    ...
    def _read_openjpeg(self, rlevel=0, verbose=False):
        ...
        if rlevel != 0:
            ...
            if rlevel < -1 or rlevel > max_rlevel:
                msg = "rlevel must be in the range [-1, %s] for this image."
                msg = msg % max_rlevel
                raise IOError(msg)
    ...
    def _populate_dparam(self, layer, rlevel, area, tile):
        ...
        if area is not None:
            if area[0] < 0 or area[1] < 0 or area[2] <= 0 or area[3] <= 0:
                ...
                msg += "lower right corner coordinates must be positive:  %s"
                raise IOError(msg % area)
            dparam.DA_y0 = area[0]
            dparam.DA_x0 = area[1]
            dparam.DA_y1 = area[2]
            dparam.DA_x1 = area[3]
...
def _validate_nonzero_image_size(nrows, ncols, component_index):
    ...
    if nrows == 0 or ncols == 0:
        ...
        msg = "Component %s has dimensions %s x %s"
        msg = msg % (component_index, nrows, ncols)
        raise IOError(msg)
...
def _unpack_colorspace(colorspace, img_array, cparams):
    ...
    if colorspace is None:
        ...
        if colorspace.lower() not in ('rgb', 'grey', 'gray'):
            msg = 'Invalid colorspace "%s"' % (colorspace)
            raise IOError(msg)
...
def _validate_compression_params(img_array, cparams):
    ...
    if cparams.cblockw_init != 0 and cparams.cblockh_init != 0:
        ...
        if ((math.log(height, 2) != math.floor(math.log(height, 2)) or
            ...
            msg = "Bad code block size (%s, %s), "
            msg += "must be powers of 2."
            raise IOError(msg % (height, width))
    ...
    if cparams.res_spec != 0:
        ...
        for j in range(cparams.res_spec):
            ...
            if ((math.log(prch, 2) != math.floor(math.log(prch, 2)) or
                ...
                msg = "Bad precinct sizes (%s, %s), "
                msg += "must be powers of 2."
                raise IOError(msg % (prch, prcw))

    # What would the point of 1D images be?
    if img_array.ndim == 1 or img_array.ndim > 3:
        msg = "%sD imagery is not allowed." % (img_array.ndim)
        raise IOError(msg)
