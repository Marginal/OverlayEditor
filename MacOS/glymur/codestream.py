...
class Codestream(object):
    ...
    def __init__(self, fptr, length, header_only=True):
        ...
        while True:
            ...
            try:
                ...
                msg = "Marker had length %s instead of expected length of 2 "
                msg += "bytes.  Codestream parsing terminated."
                warnings.warn(msg % (len(read_buffer)))
                break
            ...
            try:
                ...
                msg = 'Invalid marker id encountered at byte %d '
                msg += 'in codestream:  "0x%x"'
                msg = msg % (self._offset, self._marker_id)
                warnings.warn(msg)
                break
    ...
    def _parse_unrecognized_segment(self, fptr):
        ...
        """
        msg = "Unrecognized marker id:  0x%x" % (self._marker_id)
        warnings.warn(msg)
        cpos = fptr.tell()
        read_buffer = fptr.read(2)
        ...
        if ((next_item & 0xff00) >> 8) == 255:
            ...
            segment = Segment(id='0x%x' % (self._marker_id),
    ...
    def _parse_reserved_segment(self, fptr):
        ...
        segment = Segment(marker_id='0x%x' % (self._marker_id),
    ...
    def _parse_qcc_segment(self, fptr):
        ...
        if cqcc >= self._csiz:
            msg = "Invalid component number (%s), "
            msg += "number of components is only %s."
            msg = msg % (cqcc, self._csiz)
            warnings.warn(msg)
...
class Segment(object):
    ...
    def __str__(self):
        msg = '%s marker segment @ (%s, %s)' % (self.marker_id,
                                                self.offset,
                                                self.length)
        return msg
...
class COCsegment(Segment):
    ...
    def __str__(self):
        ...
        msg += '\n    Associated component:  %s' % (self.ccoc)
        ...
        msg += '\n        Code block height, width:  (%s x %s)'
        msg += '\n        Wavelet transform:  %s'
        msg = msg % (self.spcoc[0] + 1,
...
class CODsegment(Segment):
    ...
    def __init__(self, scod, spcod, length, offset):
        ...
        if params[3] > opj2.J2K_MAXRLVLS:
            msg = "Invalid number of resolutions (%s)."
            msg = msg % (params[3] + 1)
            warnings.warn(msg)
    ...
    def __str__(self):
        ...
        msg += '\n    Coding style:'
        msg += '\n        Entropy coder, %s partitions'
        msg += '\n        SOP marker segments:  %s'
        msg += '\n        EPH marker segments:  %s'
        msg = msg % ('with' if (self.scod & 1) else 'without',
                     ((self.scod & 2) > 0),
                     ((self.scod & 4) > 0))
        ...
        lines = ['Coding style parameters:',
                 '    Progression order:  %s',
                 '    Number of layers:  %s',
                 '    Multiple component transformation usage:  %s',
                 '    Number of resolutions:  %s',
                 '    Code block height, width:  (%s x %s)',
                 '    Wavelet transform:  %s']
        msg += '\n    '.join(lines)

        msg = msg % (_PROGRESSION_ORDER_DISPLAY[self.spcod[0]],
                     self.layers,
                     mct,
                     self.spcod[4] + 1,
                     int(self.code_block_size[0]),
...
class CMEsegment(Segment):
    ...
    def __str__(self):
        ...
        if self.rcme == 1:
            # latin-1 string
            msg += '    "%s"'
            msg = msg % (self.ccme.decode('latin-1'))
        else:
            msg += "    binary data (rcme = %s):  %s bytes"
            msg = msg % (self.rcme, len(self.ccme))
        return msg
...
class PODsegment(Segment):
    ...
    def __str__(self):
        ...
        for j in range(len(self.rspod)):
            ...
            msg += '\n    '
            lines = ['Progression change %s:',
                     '    Resolution index start:  %s',
                     '    Component index start:  %s',
                     '    Layer index end:  %s',
                     '    Resolution index end:  %s',
                     '    Component index end:  %s',
                     '    Progression order:  %s']
            submsg = '\n    '.join(lines)
            msg += submsg % (j,
                             self.rspod[j],
...
class PPMsegment(Segment):
    ...
    def __str__(self):
        msg = Segment.__str__(self)
        msg += '\n    Index:  %s'
        msg += '\n    Data:  %s uninterpreted bytes'
        msg = msg % (self.zppm, len(self.data))
        return msg
...
class PPTsegment(Segment):
    ...
    def __str__(self):
        msg = Segment.__str__(self)
        msg += '\n    Index:  %s'
        msg += '\n    Packet headers:  %s uninterpreted bytes'
        msg = msg % (self.zppt, len(self.ippt))
        return msg
...
class QCCsegment(Segment):
    ...
    def __str__(self):
        ...
        msg += '\n    Associated Component:  %s' % (self.cqcc)
        msg += _print_quantization_style(self.sqcc)
        msg += '%s guard bits' % (self.guard_bits)
...
class RGNsegment(Segment):
    ...
    def __str__(self):
        ...
        msg += '\n    ROI style:  %s'
        msg += '\n    Parameter:  %s'
        msg = msg % (self.crgn, self.srgn, self.sprgn)
...
class SIZsegment(Segment):
    ...
    def __init__(self, xy_buffer, component_buffer, length, offset):
        ...
        if numtiles > 65535:
            msg = "Invalid number of tiles (%s)." % (numtiles)
            warnings.warn(msg)
        ...
        for j, subsampling in enumerate(list(zip(data[1::3], data[2::3]))):
            if 0 in subsampling:
                msg = "Invalid subsampling value for component %s: "
                msg += "dx=%s, dy=%s."
                msg = msg % (j, subsampling[0], subsampling[1])
                warnings.warn(msg)
        self.xrsiz = data[1::3]
        self.yrsiz = data[2::3]
    ...
    def __str__(self):
        ...
        lines = ['Profile:  %s',
                 'Reference Grid Height, Width:  (%s x %s)',
                 'Vertical, Horizontal Reference Grid Offset:  (%s x %s)',
                 'Reference Tile Height, Width:  (%s x %s)',
                 'Vertical, Horizontal Reference Tile Offset:  (%s x %s)',
                 'Bitdepth:  %s',
                 'Signed:  %s',
                 'Vertical, Horizontal Subsampling:  %s']
        msg += '\n    '.join(lines)
        msg = msg % (_CAPABILITIES_DISPLAY[self.rsiz],
                     self.ysiz, self.xsiz,
...
class SOTsegment(Segment):
    ...
    def __str__(self):
        msg = Segment.__str__(self)
        msg += '\n    '
        lines = ['Tile part index:  %s',
                 'Tile part length:  %s',
                 'Tile part instance:  %s',
                 'Number of tile parts:  %s']
        msg += '\n    '.join(lines)
        msg = msg % (self.isot,
                     self.psot,
                     self.tpsot,
...
def _context_string(context):
    ...
    lines = ['Selective arithmetic coding bypass:  %s',
             'Reset context probabilities on coding pass boundaries:  %s',
             'Termination on each coding pass:  %s',
             'Vertically stripe causal context:  %s',
             'Predictable termination:  %s',
             'Segmentation symbols:  %s']
    msg += '\n            '.join(lines)
    msg = msg % (((context & 0x01) > 0),
                 ((context & 0x02) > 0),
                 ((context & 0x04) > 0),
                 ((context & 0x08) > 0),
                 ((context & 0x10) > 0),
