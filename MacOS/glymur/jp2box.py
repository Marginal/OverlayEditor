...
class Jp2kBox(object):
    ...
    def __str__(self):
        msg = "%s Box (%s)" % (self.longname, self.box_id)
        msg += " @ (%s, %s)" % (self.offset, self.length)
        return msg

    def write(self, _):
        """Must be implemented in a subclass.
        """
        msg = "Not supported for %s box." % (self.longname)
    ...
    def parse_superbox(self, fptr):
        ...
        while True:
            ...
            try:
                ...
                msg = 'Unrecognized box (%s) encountered.' % (box_id)
                warnings.warn(msg)
            ...
            if num_bytes > self.length:
                ...
                # enclosing superbox.
                msg = '%s box has incorrect box length (%s)'
                msg = msg % (box_id, num_bytes)
                warnings.warn(msg)
            elif fptr.tell() > start + num_bytes:
                # The box must be invalid somehow, as the file pointer is
                # positioned past the end of the box.
                msg = '%s box may be invalid, the file pointer is positioned '
                msg += '%s bytes past the end of the box.'
                msg = msg % (box_id, fptr.tell() - (start + num_bytes))
                warnings.warn(msg)
    ...
    def parse(fptr, offset, length):
        ...
        if method == 1:
            ...
            if numbytes < 128:
                ...
                msg += "only %s instead of 128."
                warnings.warn(msg % (numbytes), UserWarning)
                icc_profile = None
            else:
...
class _ICCProfile(object):
    ...
    """
    profile_class = {'scnr': 'input device profile',
                     'mntr': 'display device profile',
                     'prtr': 'output device profile',
                     'link': 'devicelink profile',
                     'spac': 'colorspace conversion profile',
                     'abst': 'abstract profile',
                     'nmcl': 'name colour profile'}

    colour_space_dict = {'XYZ ': 'XYZ',
                         'Lab ': 'Lab',
                         'Luv ': 'Luv',
                         'YCbr': 'YCbCr',
                         'Yxy ': 'Yxy',
    ...
    def __init__(self, read_buffer):
        ...
        minor = (data[3] & 0xf0) >> 4
        bugfix = (data[3] & 0x0f)
        header['Version'] = '%s.%s.%s' % (major, minor, bugfix)
        ...
        attr = "%s, %s, %s media polarity, %s media"
        attr = attr % ('transparency' if val & 0x01 else 'reflective',
        ...
        if read_buffer[80:84] == '\x00\x00\x00\x00':
...
class ChannelDefinitionBox(Jp2kBox):
    ...
    def __str__(self):
        ...
        for j in range(len(self.association)):
            ...
            msg += '\n    Channel %s (%s) ==> (%s)'
            msg = msg % (self.index[j], color_type_string, assn)
        return msg
    ...
    def __str__(self):
        ...
        for k in range(len(self.component_index)):
            if self.mapping_type[k] == 1:
                msg += '\n    Component %s ==> palette column %s'
                msg = msg % (self.component_index[k],
                             self.palette_index[k])
            else:
                msg += '\n    Component %d ==> %d'
                msg = msg % (self.component_index[k], k)
        return msg
...
class FileTypeBox(Jp2kBox):
    ...
    def __str__(self):
        lst = [Jp2kBox.__str__(self),
               ...
               '    Compatibility:  %s']
        msg = '\n'.join(lst)
        msg = msg % (self.brand, self.compatibility_list)
...
class ImageHeaderBox(Jp2kBox):
    ...
    def __str__(self):
        ...
        msg += '\n    Signed:  %s'
        msg += '\n    Compression:  %s'
        msg += '\n    Colorspace Unknown:  %s'
        msg = msg % (Jp2kBox.__str__(self),
...
class JPEG2000SignatureBox(Jp2kBox):
    ...
    def __str__(self):
        ...
        msg = msg % (self.signature[0], self.signature[1],
...
def _buffer2palette(read_buffer, num_rows, num_cols, bps):
    ...
    for j in range(num_cols):
        if bps[j] <= 8:
            ...
            msg = 'Unsupported palette bitdepth (%d).' % (bps[j])
            raise IOError(msg)
...
class ReaderRequirementsBox(Jp2kBox):
    ...
    def __str__(self):
        ...
        for j in range(len(self.standard_flag)):
            ...
            msg += '\n        Feature %03d:  %s' % (sfl, rrdisp)

        msg += '\n    Vendor Features:'
        for j in range(len(self.vendor_feature)):
            msg += '\n        UUID %s' % (self.vendor_feature[j])
    ...
    def parse(fptr, offset, length):
        ...
        try:
            ...
            msg = 'The ReaderRequirements box (rreq) has a mask length of %s '
            msg += 'bytes, but only values of 1, 2, 4, or 8 are supported.  '
            msg += 'The box contents will not be interpreted.'
            warnings.warn(msg % (mask_length), UserWarning)
...
class CaptureResolutionBox(Jp2kBox):
    ...
    def __str__(self):
        msg = Jp2kBox.__str__(self)
        msg += '\n    VCR:  %s' % (self.vertical_resolution)
        msg += '\n    HCR:  %s' % (self.horizontal_resolution)
        return msg
...
class DisplayResolutionBox(Jp2kBox):
    ...
    def __str__(self):
        msg = Jp2kBox.__str__(self)
        msg += '\n    VDR:  %s' % (self.vertical_resolution)
        msg += '\n    HDR:  %s' % (self.horizontal_resolution)
        return msg
...
class XMLBox(Jp2kBox):
    ...
    def parse(fptr, offset, length):
        ...
        try:
            ...
            msg += 'byte position %s (%s), but the XML was still recovered.'
            msg = msg % (offset, ude.reason)
...
class UUIDBox(Jp2kBox):
    ...
    def __init__(self, the_uuid, raw_data, length=0, offset=-1):
        ...
        elif the_uuid.bytes == 'JpgTiffExif->JP2':
    ...
    def __str__(self):
        ...
        if self.uuid == uuid.UUID('be7acfcb-97a9-42e8-9c71-999491e3afac'):
        ...
        elif self.uuid.bytes == 'JpgTiffExif->JP2':
            uuid_type = ' (Exif)'
            ...
            uuid_type = ''
            uuid_data = '%s bytes' % (len(self.data))
    ...
    def post_process(self, tagnum2name):
        ...
        for tag, value in self.raw_ifd.items():
            try:
                ...
                msg = 'Unrecognized Exif tag "%s".' % (tag)
                warnings.warn(msg, UserWarning)
                tag_name = tag
